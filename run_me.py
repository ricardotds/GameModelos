# -*- coding: utf-8 -*-
"""
Created on Thu Feb 21 22:13:27 2018
...
#    print(sns.heatmap(corr, xticklabels=corr.columns, yticklabels=corr.columns))
    ...
    df_na = df.fillna(0)
    dfFloat = df_na.astype(float)
    scaled_df = scaler.fit_transform(dfFloat)   
    ...
    final_iv, IV = data_vars(scaled_df[scaled_df.columns.difference(['target'])],df.target,max_bin)
    IV = IV.rename(columns={'VAR_NAME':'index'})
    print('----\n|IV|\n----\n{}'.format(IV.sort_values(['IV'],ascending=0)))
    print('-----\n|WOE|\n-----\n')
    #transform_vars_list = df.columns.difference(['target'])
    transform_vars_list = scaled_df.columns.difference(['target'])
    ...
    for var in transform_vars_list:
        ...
        if replace_cmd != '0':
            try:
                ...
                scaled_df[transform_prefix + var] = scaled_df[var].apply(lambda x: eval(replace_cmd))
            except:
                #df[transform_prefix + var] = df[var].apply(lambda x: eval(replace_cmd1))
                scaled_df[transform_prefix + var] = scaled_df[var].apply(lambda x: eval(replace_cmd1))
                
                
              
    # TESTE
    features = scaled_df[scaled_df.columns.difference(['target'])]
